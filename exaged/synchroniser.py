...
class Synchronizer:
    ...
    def synchronize_devis(self):
        ...
        devis = self.api.restv1(
            GET(
                "crm/Quotations?$select=QuotationID,OrderAccount,QuotationNumber,YourRef,Description"
            )
        )
    ...
    def synchronize_commandes(self):
        ...
        commandes = self.api.restv1(
            GET(
                "salesorder/SalesOrders?$select=SalesOrderLines,OrderID,Description,OrderedBy,YourRef,OrderNumber,Status,StatusDescription,Modified&$expand=SalesOrderLines"
            )
        )
